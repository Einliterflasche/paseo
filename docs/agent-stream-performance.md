...
- **Cumulative subagent logs share immutable text between canonical rows.** Providers can publish a complete growing log on every update. Retaining each expanded version caused quadratic heap growth even though the UI showed only the latest version. The timeline store owns sharing for both managed agents and provider children; each historical row still reads its exact original text. Do not cache expanded historical strings or spread their lazy detail objects into a long-lived store. Checkpoint export preserves the shared representation; see [controlled restart recovery](restart-recovery-plan.md#3-commit-the-snapshot). Run collectors and subscribers receive the same canonical versions. Response paging accounts for escaped text bytes before expanding lazy logs, using the existing physical socket capacity; see [timeline sync](timeline-sync.md).
...
- **Run collector retention:** `packages/server/src/server/agent/manager-retention.posix.test.ts` exercises the real Codex adapter through `AgentManager.runAgent` and a new-agent schedule while each run is still active. The companion `scripts/reproduce-manager-retention.mts` records retained heap separately from logical history size.
- **Cumulative log retention:** `packages/server/src/server/agent/timeline-retention.posix.test.ts` runs the real Codex translation, coalescer, canonical/child stores, reads, and checkpoint round trip in a separate GC-enabled process. `scripts/reproduce-timeline-retention.ts` also accepts larger workloads for incident comparisons. Its memory thresholds are test budgets, not product retention limits; exact historical rows and UTF-16 text must survive.
